...
import {
  ...
  listActiveVacancies,
  deleteJobApplications, // Ensure this is exported from your API file
...
function JobApplicationsPage() {
  ...
  const fetchApps = async (pageNo = 1, currentSize = pageSize) => {
    ...
    try {
      ...
      if (resp.success) {
        ...
        setPage(pageNo);
        setSelectedIds([]); // Clear selection when page changes or filters apply
      }
    } catch (err) {
  ...
  const handleDelete = async (ids) => {
    if (!window.confirm(`Are you sure you want to delete ${ids.length} application(s)?`)) return;
    try {
      setLoading(true);
      const res = await deleteJobApplications(ids);
      if (res.success) {
    ...
    <div className="container">
      <style>{`
        ...
        @media (max-width: 768px) {
          .filters-left { flex-direction: column; align-items: stretch !important; }
